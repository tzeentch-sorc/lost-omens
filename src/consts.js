...
// Дев-сборка: `npm start`, в том числе открытая внутри VK через тоннель. Отвечает на
// вопрос «можно ли включать отладочные послабления» — DEBUG_MODE, подробный лог.
// К доступности VK отношения не имеет: внутри тоннеля сборка дев, а VK работает.
export const DEV_BUILD = process.env.NODE_ENV === 'development';

// Отдельный вопрос: доступен ли VK. Если нет — пользователя, мастеров и фотографии
// берём из config.json. Каждое из трёх условий закрывает свой случай:
//
// DEV_BUILD    — прод-сборка не подставляет моки никогда, что бы ни было в переменных.
//
// isEmbedded() — дев-сервер, открытый внутри VK через dev.geekmo.ru, работает с
//                настоящими данными. Это повседневный путь разработки (tunnel.md),
//                и раньше его закрывало сравнение с localhost: адрес тоннеля не
//                localhost, поэтому моки выключались сами. Проверка по NODE_ENV
//                этого не умеет — она одинакова и в браузере, и внутри VK, —
//                а bridge.isEmbedded() отвечает ровно на нужный вопрос:
//                iframe у веба, webview у мобильного приложения.
//
// REACT_APP_MOCK_VK=false — ручное отключение, чтобы пройти боевым путём локально.
//
// Имя хоста при этом больше не участвует: дев-сервер, открытый с телефона по
// локальной сети, отдаёт моки, как и на localhost.
